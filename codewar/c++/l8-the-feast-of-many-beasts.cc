...
// better solution
bool feast1(std::string beast, std::string dish) {
  return beast.front()==dish.front() && beast.back()==dish.back();
}

bool feast(std::string beast, std::string dish) {
  bool beg = (*(beast.begin()) == *(dish.begin()));
  bool end = (*(beast.end() - 1) == *(dish.end() - 1));

  return beg && end;
}

int main() {
  std::cout << feast1("great blue heron", "garlic naan") << std::endl;
  return 0;
}